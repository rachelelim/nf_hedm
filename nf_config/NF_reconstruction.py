...
class NF_ReconstructionConfig(Config):
    ...
    def tomography(self):
        ...
        if key is not None:
            parms = dict(mask_filepath=self._cfg.get('NF_reconstruction:tomography:mask_filepath'),
                         vertical_motor_position=self._cfg.get(
                             'NF_reconstruction:tomography:vertical_motor_position'),
                         use_single_layer=self._cfg.get('NF_reconstruction:tomography:use_single_layer'))
            # parms = dict{mask_filepath = self._cfg.get('NF_reconstruction:tomography:mask_filepath'),
            #     self._cfg.get('NF_reconstruction:tomography:vertical_motor_position'),
            #     self._cfg.get('NF_reconstruction:tomography:use_single_layer')}
    ...
    def beam_stop(self):
        key = self._cfg.get(
            'NF_reconstruction:beam_stop:beam_stop_filepath', None)
        if key == 'None':
            key = None
        if key is not None:
            return key
    ...
    def missing_grains(self):
        ...
        if key is not None:
            parms = dict(reconstructed_data_path=self._cfg.get('NF_reconstruction:missing_grains:reconstructed_data_path'),
                         ori_grid_spacing=self._cfg.get(
                             'NF_reconstruction:missing_grains:ori_grid_spacing'),
                         confidence_threshold=self._cfg.get(
                             'NF_reconstruction:missing_grains:confidence_threshold'),
                         low_confidence_sparsing=self._cfg.get(
                             'NF_reconstruction:missing_grains:low_confidence_sparsing'),
                         errode_free_surface=self._cfg.get(
                             'NF_reconstruction:missing_grains:errode_free_surface'),
                         coord_cutoff_scale=self._cfg.get(
                             'NF_reconstruction:missing_grains:coord_cutoff_scale'),
                         iter_cutoff=self._cfg.get(
                             'NF_reconstruction:missing_grains:iter_cutoff'),
